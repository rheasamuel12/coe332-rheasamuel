...
#Check the hemisphere of the each landing
def check_hemisphere(latitude: float, longitude: float) -> str:    # type hints
    ...
    return(location)
#Counting the amount of classes (Used chatGPT to fix errors in this function)
def count_classes(a_list_of_dicts, a_key_string):
    ...
    for i in a_list_of_dicts:
        ...
        if recclass:
            total_classes += 1
            if recclass in classes_count: #Checks if its already in list
                classes_count[recclass] += 1
            else:
                classes_count[recclass] = 1 #If not, adds it and adds count 1
...
# Include argparse to accept filename as an argument 
    ...
    parser.add_argument('file', help='JSON file with meteorite_landing') 
    ...
    # Count classes
    total_classes, classes_count = count_classes(ml_data['meteorite_landings'], 'recclass')
    ...
    #Displays the average mass of the meteorite landings:
    print("\n3. The average mass of the meteorite landings is", compute_average_mass(ml_data['meteorite_landings'], 'mass (g)'),"g")

    #Displays how many meteorite landings are in specific places
    NE = 0
    NW = 0
    SE = 0
    SW = 0
